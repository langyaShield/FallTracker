"""Use cases for data backup: full export/import + Tencent COS cloud backup.

The service owns transaction boundaries and all ORM persistence. HTTP handlers
should only translate requests into these use cases.
"""
...
class BackupService:
    """Encapsulates all backup/restore logic and transaction management."""
    ...
    def _gather_backup_data(self, uid: int) -> dict:
        """Collect all user data into a dict ready for JSON serialization."""

        # UserSettings
        settings = self.db.query(UserSettings).filter(UserSettings.user_id == uid).first()
        user_settings = _model_to_dict(settings) if settings else {}

        # CrawlerConfig
        configs = self.db.query(CrawlerConfig).filter(CrawlerConfig.user_id == uid).all()
        crawler_configs = []
        for c in configs:
            d = _model_to_dict(c)
            d["_old_id"] = c.id
        ...
        # CrawlerResult
        results = self.db.query(CrawlerResult).join(
        ...
        # Resume
        resumes = self.db.query(Resume).filter(Resume.user_id == uid).all()
        resume_list = []
        for r in resumes:
            d = _model_to_dict(r)
            d["_old_id"] = r.id
        ...
        # Delivery
        deliveries = self.db.query(Delivery).filter(Delivery.user_id == uid).all()
        delivery_list = []
        for d in deliveries:
            dd = _model_to_dict(d)
            dd["_old_id"] = d.id
        ...
        # Notification
        notifications = self.db.query(Notification).filter(Notification.user_id == uid).all()
        notification_list = [_model_to_dict(n) for n in notifications]

        # ProfileField
        profile_fields = self.db.query(ProfileField).filter(ProfileField.user_id == uid).all()
        profile_field_list = [_model_to_dict(pf) for pf in profile_fields]

        # Bookmark
        bookmarks = self.db.query(Bookmark).filter(Bookmark.user_id == uid).all()
    ...
    def _import_backup_data(self, uid: int, data: dict) -> dict:
        ...
        delivery_ids = [row[0] for row in self.db.query(Delivery.id).filter(Delivery.user_id == uid).all()]
        config_ids = [row[0] for row in self.db.query(CrawlerConfig.id).filter(CrawlerConfig.user_id == uid).all()]

        self.db.query(Bookmark).filter(Bookmark.user_id == uid).delete()
        self.db.query(ProfileField).filter(ProfileField.user_id == uid).delete()
        self.db.query(Notification).filter(Notification.user_id == uid).delete()
        self.db.query(Review).filter(Review.user_id == uid).delete()
        if delivery_ids:
            self.db.query(DeliveryNote).filter(
                DeliveryNote.delivery_id.in_(delivery_ids)
            ).delete(synchronize_session=False)
            self.db.query(InterviewEvent).filter(
                InterviewEvent.delivery_id.in_(delivery_ids)
            ).delete(synchronize_session=False)
        self.db.query(Delivery).filter(Delivery.user_id == uid).delete()
        if config_ids:
            self.db.query(CrawlerResult).filter(
                CrawlerResult.config_id.in_(config_ids)
            ).delete(synchronize_session=False)
        self.db.query(CrawlerConfig).filter(CrawlerConfig.user_id == uid).delete()
        self.db.query(Resume).filter(Resume.user_id == uid).delete()
        self.db.query(UserSettings).filter(UserSettings.user_id == uid).delete()
        self.db.commit()
    ...
    def export_data(self, user_id: int) -> tuple:
        """导出当前用户全量数据。返回 (content_bytes, filename)。"""
        data = self._gather_backup_data(user_id)
    ...
    def import_data(self, user_id: int, content: bytes) -> dict:
        """导入 JSON 备份文件恢复数据（覆盖模式：先清空再导入）。返回 stats。"""
        try:
        ...
        stats = self._import_backup_data(user_id, data)
        return stats

    # ── COS Cloud Backup ──

    def upload_to_cos(self, user_id: int) -> dict:
    ...
    def list_cos_backups(self, user_id: int) -> list:
        """列出 COS 上的备份文件。"""
        s = self.db.query(UserSettings).filter(UserSettings.user_id == user_id).first()
        if not s:
            raise HTTPException(status_code=400, detail="请先在设置中配置腾讯云 COS 参数")
    ...
    def delete_cos_backup(self, user_id: int, file_key: str) -> dict:
        """删除 COS 上的指定备份文件。"""
        s = self.db.query(UserSettings).filter(UserSettings.user_id == user_id).first()
        if not s:
            raise HTTPException(status_code=400, detail="请先在设置中配置腾讯云 COS 参数")
    ...
    def rename_cos_backup(self, user_id: int, file_key: str, new_name: str) -> dict:
    ...
    def restore_from_cos(self, user_id: int, file_key: str) -> dict:
    ...
    def _auto_backup_user(self, s: UserSettings) -> None:
        """Perform auto-backup for a single user if the interval has elapsed."""
        from app.models import User as UserModel

        # Verify user exists and is not disabled
        user = self.db.query(UserModel).filter(UserModel.id == s.user_id, UserModel.is_disabled.is_(False)).first()
        if not user:
            return
        ...
        data = self._gather_backup_data(s.user_id)
        content = json.dumps(data, ensure_ascii=False, indent=2).encode("utf-8")
